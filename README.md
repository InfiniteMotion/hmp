...
完整版本历史与变更日志见 **[ROADMAP](ROADMAP.md)**。**最新可下载安装包：v7.1.0**；v7.2.x（Agent 化 + 官网改版）代码已在 master，发布通道修复中，**进行中的版本号以 `gradle.properties` 与 ROADMAP 为准**（`./gradlew syncVersion` 统一下发）；**后续待做：方向 C 播放增强、F10 语音会话（挂起）**。
