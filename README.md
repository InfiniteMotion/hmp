...
- **AI 音乐 Agent**：自然语言对话与曲库问答、歌单生成与播放控制、AI 电台（播完基于上下文自动续队列）、听歌报告与用户认识画像，破坏性操作经 UI 确认与权限分级护栏
...
完整版本历史与变更日志见 **[ROADMAP](ROADMAP.md)**。**最新可下载安装包：v7.1.0**；v7.2.0 的代码已合入 master 但未产出发布物（CI 三平台构建失败、无 tag）；**v7.2.1 发布中**（`release/7.2.1` → master）—— 修好发版通道，并作为方向 B Agent 化（F1–F14）与官网改版的**首个可安装包**。**后续待做：方向 C 播放增强、F10 语音会话（挂起）**。
