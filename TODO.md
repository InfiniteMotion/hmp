...
- [x] **R32** 自有发行版缺 FFmpeg 的**真根因**（2026-09-24 复核后改写，此前结论不准）：不是「找不到 runtime 根」，而是**注入点选错 + 时机太晚**。CMP 1.11 的任务图里只有 macOS 的 `packageDmg` 依赖 `createDistributable`，Windows(`packageMsi`)/Linux(`packageDeb`) 各自起 jlink+jpackage、**从不执行 `createDistributable`** → 挂在它 `finalizedBy` 上的 `injectFFmpeg` 连触发都没有（v7.1.0 三次成功 job 的日志已实证：Windows/Linux 任务序里没有 `createDistributable`，任务静默通过）；macOS 虽触发，但 finalizer 排在 `packageDmg` **之后**，DMG 早已封包。→ **v7.1.0 及更早的 MSI/DEB/DMG 全部没有内置 ffmpeg**，只因开发机装了 ffmpeg（运行时回落 PATH/`/opt/homebrew/bin`）而未被发现。修法改为走 jpackage 的 `--resource-dir`（CMP 对每种格式都传，落点 `$APPDIR/resources/ffmpeg`，与任务顺序无关），运行时优先按 `compose.application.resources.dir` 解析，`java.home/bin` 仅作回退。本机 macOS 已验证资源目录落点；Windows/Linux 由 CI 产物断言复验（`dpkg -c` / app image 断言此前正是暴露本问题的探针，保留）
- [ ] **R36** 产物断言需要能区分「缺 ffmpeg」与「断言路径假设不成立」：Windows 的 verify 步查 `compose/binaries/main/app`，而该目录在 Windows 上**从来不存在**（根因同 R32），失败信息 `find: No such file or directory` 会把人带向错误方向。修 R32 后此断言应改为查能稳定存在的落点（app image 或解包 MSI），并让「目录不存在」与「目录在但没 ffmpeg」给出不同报错
...
- [x] **R34** 版本声明由「校验」升级为「生成」—— 已完成：`syncVersion`（`build.gradle.kts` 的 `versionSites` 表，13 处声明点）以 `gradle.properties` 为唯一真源下发，**versionCode 由 versionName 推导**（杜绝手写算错），`checkReleaseConsistency` 与它共用同一张表；本机已验：幂等（二次运行全 `[=]`）、负测（改坏 `DEVELOP.md` 即红）、无行尾churn。**剩余没做**：把 iOS 版本从 `pbxproj` + `Info.plist` 迁到一份 `.xcconfig`（需 macOS 上 xcodegen 验，本机改不了也验不了），迁完可从表里摘掉 pbxproj 那行
- [ ] **R35** 版本历史连续性无人守：v7.2.2 那次 bump 直接把我写的 `### v7.2.1` 条目覆盖掉了，ROADMAP 现在没有 v7.2.1 一节，而 `checkReleaseConsistency` 只校验**当前版本**有没有条目，抓不到中间缺的。→ 让 `syncVersion`/校验顺带检查「最新 tag → 当前版本」之间每个版本号都有 `### vX.Y.Z` 条目，缺的报告出来（v7.2.1 这一条要么补写、要么在 v7.2.3 条目里注明它未独立发布）
