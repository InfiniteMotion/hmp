...
**进行中**：**F11 后台生命周期**（L1 `AgentRuntime` 所有者 + L2 Android 自持前台服务**已落**；余 L3 iOS 有限保活 / L5 Desktop 收口）
**待做**：**F10 语音会话**（`RealtimeVoiceTransport` + 语音会话；独立阶段，可整体延期）· **F12 Token 计量与窗口治理**（T1 真值 + 账本 → T2 分账视图 → T3 窗口 → T4 组装策略；设计已定稿，待开工）
...
| [F11](f11-后台生命周期.md) | 后台生命周期 | **L1 AgentRuntime 所有者** ✅ → **L2 Android 自持前台服务** ✅ → **L3 iOS 有限保活** → **L5 Desktop 收口**（~~L4 电台快照落盘~~ **已撤销**）| 插叙 | 🟡 进行中（L1/L2 已落）| — | — |
| [F12](f12-token计量与窗口.md) | Token 计量与窗口治理 | **T1 计量打真值 + 落账本（`token_ledger`）** → **T2 分账视图（按 Agent / 端点 / 时间）** → **T3 窗口治理（固定 64K + 前置护栏）** → **T4 上下文组装策略** → T5 成本可见（后置）；~~T2b 配额接线~~ **候补** | 插叙 | ⬜ 待开工（设计已定稿）| — | — |
