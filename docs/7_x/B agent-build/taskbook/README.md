...
F1 规划与地基 ──▶ F2 锚点与协议 ──▶ F3 工具与引擎 ──▶ F4 对话与清债
                     ...
                     F6 电台与富化 ──▶ F7 Hello 呈现 ──▶ F8 编排内核与收尾
                                                                  │
                                                                  ▼
                                              F9 报告与语音 · A0 Capability 统一化（见下）
                                                                   │
                                                                   ▼
                                               F9 报告与语音（T0 → A0 → T1-T4）
```
...
- `F9-A0 Capability 统一化`（F9 插入的插叙阶段，不占里程碑编号）—— 统一 SubAgent 能力面（`Capability` 接口 + `CapabilityStatusTool`）、清理 DJ 工具绕路债；依赖 F6 + F8（SubAgent 体系稳定后收口）
- `F9-T1/T2` 依赖 `F6 + F8`；语音档（T3/T4）为独立 gate，可整体延期不影响 v1 完整性
...
**已完成**：F1 → F2 → F3 → F4 → F5 → F6 → F7 → F8 → **F9-T0 用户认识模块（含 T0a + T0b，契约 v3.4，9 月 16 日全量落地）** → **F9-A0 Capability 统一化（插叙，9 月 16 日）**
**进行中**：—
**待做**：F9-T1/T2 报告角色与伙伴设置页；F9-T3/T4 语音（独立 gate）
...
| [F9](f9-报告与语音.md) | 报告与语音 | **T0 用户认识（T0a / T0b）** → **A0 Capability 统一化** → T1/T2 → T3/T4 | 主轴+插叙 | 🟠 T1/T2 待开工 | T0: 2026-09-16 · A0: 2026-09-16 | — |

**已知验收缺口**（不阻塞主线，需补）：

- 本地化（14 语言）横切项未做（叙事段 / 报告页所需的文案键随 F9-T1 的 UI 部分落地时一并在 `values/` 落地，其余 13 语言待同步）
...
| [F9](f9-报告与语音.md) | **T0 用户认识模块**（侧写层 + 证据层两层画像 / 两个平级建模器：曲库两阶段 + 行为定时聚合 / 分级记忆 L1-L3 / **对话侧写门槛放开**但「对话给假设、行为给确证」/ 认知准入三档 / **明确拒绝 MBTI** / C2 再修订）+ 报告角色 + 伙伴设置页 + 语音档 | 用户认识模块（T0a 地基+MVP / T0b 后置）、`user_profile_evidence` + `user_profile_portrait`（Room v6）、报告叙事段、六分区设置页；语音为独立 gate |
...
| [`../design/agent-profile.md`](../design/agent-profile.md) | **用户认识模块（画像）唯一依据**（v3.1）：两层数据模型（含迁移 DDL） / 槽位闭集（谓词即槽位限定名） / 分级记忆与承载字段 / C2 定稿（画像可读、**仅供参考**非指令） / 审计四问 / 三写入三读取 / 验收剧本 P1-P21 |
