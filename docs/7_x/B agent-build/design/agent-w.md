...
| #  | 组件                   | 当前状态                                                            | 引擎依赖                                       | W2 做什么                                                           |
| -- | -------------------- | --------------------------------------------------------------- | ------------------------------------------ | ---------------------------------------------------------------- |
| ~~C1~~ | ~~CompanionCapsule 徽标态~~ | ⛔ **已废止（2026-09-19）**：徽标圆点方案整体放弃——通道从未接线（事件发过、胶囊从未渲染），`PresenceBus.CompanionBadge`/`badgeState` 已删除 | — | **不做什么**：不再加 `BadgeOverlay` / 不再渲染红点·脉冲圈·绿点。电台运行态改由胶囊**电台态**承载（直接订阅 `radioState` → 换图标 + 恒定高亮 + 长按停止电台），见 `agent-radio.md` §3.3 |
| ~~C2~~ | ~~AgentQuickSheet 轻量浮层~~ | ⛔ **已移除（2026-09-19）**：该表面整体删除（组件文件 + 全部入口 + 2 条文案键）。理由：一句话输入的价值不足以撑起独立表面，它只是"进对话页"的中转，却带来一套平行的输入态/关闭手势/资源文案 | — | **不做什么**：不再有浮层；长按胶囊 / 播放页「对话」按钮 / C 键 一律**直达对话页**（见 `agent.md` §5.2.4）|
...
| C5 | C 键全局监听              | 已做（2026-09-19 改语义）| — | AppRoot `onKeyEvent` 拦截 C 键 → **直达对话页**（原为唤起浮层；浮层已移除）|
...
| C8  | 「对话」按钮 | PlayerScreen（PlaybackControlsButtons） | 主行三键 + 副行五键                                                  | —（浮层已移除 2026-09-19）                               | 副行重排加「对话」按钮 → **直接进对话页**（原为唤起浮层；现已统一为"找伙伴=进对话页"）                   |
