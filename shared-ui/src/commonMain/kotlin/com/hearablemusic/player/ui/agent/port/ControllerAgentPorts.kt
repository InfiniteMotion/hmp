/**
 * `ui/agent/port/` —— **agent 端口的 UI 侧实现**。
 *
 * 这些类是 `:shared` 里 `com.hmp.domain.agent.port.*` 的**实现**，不是任何页面的 UI：
 * 它们的存在理由是把 UI/平台层的东西（[com.hearablemusic.player.ui.platform.PlaybackController]）
 * 桥接给领域层，因此**按端口归属**（这里是 `agent/port/`），而不是按使用它们的页面归属。
 *
 * 原先放在 `chat/` 是因为对话页是最早的消费者；但 Koin 注册在全局模块，
 * 任何页面都可能用到 —— 放在某个页面包里会让"去哪找端口实现"变成猜谜。
 */

package com.hearablemusic.player.ui.agent.port
 ...
 * 都视为曲间空白点，供 HelloSubAgent 刷新问候卡。
 *
...
class ControllerPlaybackCommandPort(
     ...
     * 用于 MasterAgent 转发 PresenceBus.DjBlank，供门面刷新问候卡。
