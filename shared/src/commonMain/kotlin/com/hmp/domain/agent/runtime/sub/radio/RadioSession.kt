...
/** 触发原因。只用于日志与审计，不参与判断。 */
internal enum class RadioTriggerCause { SETTLED, QUEUE_LOW }

/** 模型唯一能选的三个动作。 */
internal enum class RadioAction { NONE, APPEND, REPLACE }
 ...
 */
internal data class RadioVerdict(
...
internal data class RadioQueueEntry(
    val musicId: Long,
    val title: String,
)

/** 判断时附加的**系统状态**（不属于用户行为，因此不进记录）。 */
internal data class RadioContextSnapshot(
    /** 当前在播曲目 —— 用于约束「不许动它」 */
 ...
 */
internal data class RadioConversation(
 ...
 * 两者都要带，复用才算真的"续上上次对话"。
 */
internal data class RadioSessionArchive(
 ...
 */
internal const val REUSE_BACKSTOP_MS = 12 * 60 * 60 * 1000L

internal class RadioSession(
    ...
    companion object {
        ...
        fun systemPrompt(
            ...
            if (preferredLang != null) {
                val base = com.hmp.domain.agent.runtime.i18n.resolvePrompt(
                    key = "radio.host",
