        ...
        all.take(500).forEach { info ->
            info.music.id?.let { id ->
                runCatching { repo.getMusicLabels(id).forEach { ml ->
                    val name = ml.label.name
                    labelCounter[name] = (labelCounter[name] ?: 0) + 1
                } }
            }
        }
...
/**
 * `dj_queue_replace_next` 的结构化回传载荷（`ToolResult.detail`）。
 *
 * RadioSubAgent 用它在 ReAct 循环结束后收口 `currentPlaylist`：
 * 只有真的写进播放队列的曲目（按传入顺序、剔除查不到的 ID）才会出现在这里，
 * 避免"LLM 说的"与"实际入队的"不一致。
 */
@Serializable
data class DjQueuePayload(
...
data class DjQueuedTrack(
    ...
    val title: String,
    val artist: String = "",
    /** LLM 通过 `reasons` 参数同序给出的推荐理由；未给或为空时 null */
    val why: String? = null,
)

private val djJson = Json { ignoreUnknownKeys = true; encodeDefaults = true }
...
class DjQueueReplaceNextTool(
    ...
    override val params = listOf(
        ...
        StringListParam(
            name = "reasons",
            description = "与 music_ids 同序的推荐理由（每首一句、20 字以内）；可省略",
            required = false,
        ),
    )
    ...
    override suspend fun run(args: ToolArgs): ToolResult {
        ...
        val reasons = args.optionalStringList("reasons")
        ...
        // getMusicInfoByIds 不保证顺序——按传入的 music_ids 重排，理由按下标对齐
        val byId = existing.associateBy { it.music.id }
        val payload = DjQueuePayload(
            tracks = ids.mapIndexedNotNull { index, id ->
                val info = byId[id] ?: return@mapIndexedNotNull null
                DjQueuedTrack(
                    id = info.music.id,
                    title = info.music.title,
                    artist = info.music.artist,
                    why = reasons.getOrNull(index)?.trim()?.takeIf { it.isNotEmpty() },
                )
            },
        )
        if (payload.tracks.isEmpty()) return ToolResult.failure("传入的 music_ids 都找不到对应的歌曲")

        // 调用 REPLACE_QUEUE —— port 层已保证"不打断当前播放"
        // AGENT_INTERNAL：DJ 自己的编排动作，不能被 MasterAgent 误判为用户意图
        val (ok, text) = port.execute(
            PlaybackCommand.REPLACE_QUEUE(payload.tracks.map { it.id }),
            CommandSource.AGENT_INTERNAL,
        )
        return if (ok) {
            val preview = payload.tracks.take(3).joinToString("、") { it.title }
            val more = if (payload.tracks.size > 3) "... 等 ${payload.tracks.size} 首" else ""
            ToolResult.success(
                summary = "队列已更新：$preview$more（当前播放不受影响）",
                detail = djJson.encodeToString(DjQueuePayload.serializer(), payload),
            )
        } else {
