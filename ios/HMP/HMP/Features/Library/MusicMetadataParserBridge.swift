...
class MusicMetadataParser: MetadataParserBridgeParser {
    func parse(filePath: String) -> MusicMetadata? {
        HmpLog.d(HmpTag.libraryMetadata, "🏷️ parsing: \(filePath)")
        ...
        HmpLog.d(HmpTag.libraryMetadata, "🏷️ result: title=\(result.title), artist=\(result.artist), album=\(result.album), lyrics=\(lyrics != nil ? ")
        return result
    }
    ...
    private func parseFlacVorbisLyrics(filePath: String) -> String? {
        guard let data = try? Data(contentsOf: URL(fileURLWithPath: filePath), options: .alwaysMapped) else {
            HmpLog.e(HmpTag.libraryMetadata, "🏷️ FLAC: failed to read file")
            return nil
        }
        ...
        guard bytes.count > 8,
            ...
            HmpLog.e(HmpTag.libraryMetadata, "🏷️ FLAC: not a valid FLAC file")
            return nil
        }
    ...
    private func parseVorbisComments(bytes: [UInt8], offset: Int, length: Int) -> String? {
        ...
        for _ in 0..<commentCount {
            ...
            if lyricsKeys.contains(key) {
                HmpLog.d(HmpTag.libraryMetadata, "🏷️ FLAC Vorbis: found lyrics via key '\(key)'")
                return value
            }
        }
        return nil
